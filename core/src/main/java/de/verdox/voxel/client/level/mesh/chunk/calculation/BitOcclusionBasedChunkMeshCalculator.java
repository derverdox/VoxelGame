...
public class BitOcclusionBasedChunkMeshCalculator implements ChunkMeshCalculator {
    ...
    public void calculateChunkMesh(TerrainFaceStorage.ChunkFaceStorage blockFaces, TerrainChunk chunk, int lodLevel) {
        ...
        OccupancyMask occupancyMask = lodLevel == 0 ? chunk.getChunkOccupancyMask() : chunk
    ...
    private static void generateBlockFacesForDirection(TerrainManager terrainManager, TerrainFaceStorage.ChunkFaceStorage blockFaces, byte lodLevel, Direction d, int sx, int sy, OccupancyMask occupancyMask, OccupancyMask neighborOcclusionMap, int sz, Chunk lookupChunk) {
            ...
            for (int lightLookupY = 0; lightLookupY < sy; lightLookupY += 1) {
                ...
                while (bits != 0L) {
                    ...
                    for (var face : faces) {
                        ...
                        blockFaces.generateFace(
                                terrainManager,
                                lookupChunk, name, face,
                                lodLevel,
                                lightLookupX,
                                lightLookupY,
                                lightLookupZ);
