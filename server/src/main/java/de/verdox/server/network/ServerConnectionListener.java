...
public class ServerConnectionListener extends Listener {
    ...
    public void receiveClientLoadChunk(Connection connection, ClientLoadChunkPacket e) {
        ...
        optionalWorld.get().getChunkMap().getOrCreateChunkAsync(e.getChunkX(), e.getChunkY(), e.getChunkZ(), serverChunk -> {
            connection.sendTCP(ServerChunkPacket.fromGameChunk(serverChunk));
        });
    }
}
